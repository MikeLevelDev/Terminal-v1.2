...
async function intro() {
  ...
  await type(
    [">SET FILE/PROTECTION=OWNER:RWED ACCOUNTS.F", ">SET HALT RESTART/MAINT"],
    { newlineWait: 200 },
    intro
  );
...
async function game() {
  ...
  return new Promise(async (resolve) => {
    ...
    for (let rowNr = 0; rowNr < ROWS; rowNr++) {
      ...
      let chars = text
        .slice(rowNr * CHARS_PER_ROW, rowNr * CHARS_PER_ROW + CHARS_PER_ROW)
        .map(charToSpan);
    ...
    specials.forEach((special) => {
      special.addEventListener("click", (event) => handleSpecial(event.target));
    });
...
function getActiveSpans(el) {
  ...
  if (word) {
    ...
    wordSpans = [...document.querySelectorAll(`[data-special="${special}"`)];
  }
...
*/
  ...
  console.log(
    "After pause... Shouldn't be able to do anything till this shows up"
  );
  //resolve();
  /*});
	}*/
